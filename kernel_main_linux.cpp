...
static char aion_input[128] = {0};
static int aion_input_len = 0;
static bool aion_input_focus = false;
char os_text_input_buf[256] = {0};
bool os_text_input_enter = false;
...
RadioStation radio_stations[100] = {
    {"0n KPOP", "KPOP", "https://0n-kpop.radionetz.de/0n-kpop.mp3", false},
    {"Zeno FM Kpop", "KPOP", "https://stream.zeno.fm/66gp9qukxaquv", false},
    {"Nightride FM", "Synthwave", "http://stream.nightride.fm:8443/nightride.m4a", false},
    {"Synthwave Radio", "Synthwave", "http://streaming.radionomy.com/Synthwave", false},
    {"Classic FM", "Classical", "http://media-ice.musicradio.com/ClassicFMMP3", false},
    {"Venice Classic Radio", "Classical", "http://174.36.206.197:8000/stream", false},
    {"BBC World Service", "News", "http://stream.live.vc.bbcmedia.co.uk/bbc_world_service", false},
    {"Laut.fm LoFi", "LoFi", "https://stream.laut.fm/lofi", false},
    {"RauteMusik House", "House", "https://breakz-2012-high.rautemusik.fm/?ref=rb", false}
};
int num_radio_stations = 9;
int current_radio_station = 0;
bool radio_is_playing = false;
bool media_search_active = false;
int media_scroll_y = 0;
...
extern "C" bool os_vsync_enabled = false;
extern "C" bool os_aion_heuristic = true; // Aion Zero-Day Heuristic
extern "C" bool os_auto_update = false; // Aion Auto-Update
extern "C" char aion_prediction[256] = "AION: System initialisiert. Warte auf Analyse..."; // Aion Prediction UI
#include <unistd.h>
#include <signal.h>
#include <time.h>
#include <sys/types.h>
...
// --- AION 3D AVATAR HOST INTEGRATION ---
extern "C" uint32_t aion_window_x = 0;
extern "C" uint32_t aion_window_y = 0;
...
void PutPixelAlpha(uint32_t x, uint32_t y, uint32_t color, float alpha) {
    ...
    uint32_t a_val = (uint32_t)(alpha * 255.0f);
    uint32_t inv_a = 255 - a_val;
    
    uint32_t r = (((color >> 16) & 0xFF) * a_val + ((bg >> 16) & 0xFF) * inv_a) >> 8;
    uint32_t g = (((color >> 8) & 0xFF) * a_val + ((bg >> 8) & 0xFF) * inv_a) >> 8;
    uint32_t b = ((color & 0xFF) * a_val + (bg & 0xFF) * inv_a) >> 8;
...
void DrawLineAlpha(int x0, int y0, int x1, int y1, uint32_t color, float alpha) {
    int dx = (x1 > x0) ? (x1 - x0) : (x0 - x1);
    int dy = (y1 > y0) ? (y1 - y0) : (y0 - y1);
    int sx = (x0 < x1) ? 1 : -1;
    int sy = (y0 < y1) ? 1 : -1;
    ...
    for (;;) {
        PutPixelAlpha(x0, y0, color, alpha);
        if (x0 == x1 && y0 == y1) break;
        e2 = err;
        if (e2 > -dx) { err -= dy; x0 += sx; }
        if (e2 < dy) { err += dx; y0 += sy; }
...
void DrawDisc3D(Vec3 center, float radius, uint32_t ocean_color, uint32_t ice_color, int cx, int cy) {
    if (radius > 780.0f) radius = 780.0f; // Nur Ring 1 (Ozean + Eiswand), Nebel macht GPU
    float sy = bare_sin(global_cam_rot_y); float cy_rot = bare_cos(global_cam_rot_y);
    float sx = bare_sin(global_cam_rot_x); float cx_rot = bare_cos(global_cam_rot_x);
    auto rot = [&](Vec3 p) -> Vec3 {
        float nx = p.x * cy_rot - p.z * sy; float nz = p.x * sy + p.z * cy_rot; p.x = nx; p.z = nz;
        float ny = p.y * cx_rot - p.z * sx; nz = p.y * sx + p.z * cx_rot; p.y = ny; p.z = nz; return p;
    };
    
    int num_rings = (int)(radius / 2.5f); 
    if (num_rings > 320) num_rings = 320;
    
    for (int ring = 1; ring <= num_rings; ring++) {
        float t = (float)ring / (float)num_rings;
        float r = radius * t;
        
        float a_step = 6.0f / (r < 10.0f ? 10.0f : r); 
        if (a_step < 0.005f) a_step = 0.005f; 
        if (a_step > 0.1f) a_step = 0.1f;
        
        for (float a = 0.0f; a < 6.28318f; a += a_step) {
            float next_a = a + a_step;
            
            uint32_t color = ocean_color;
            if (r >= 750.0f && r < 780.0f) {
                color = ice_color; // Antarktis / Ice Wall
            }
            
            Vec3 p1 = { center.x + bare_cos(a)*r, center.y, center.z + bare_sin(a)*r };
            Vec3 p2 = { center.x + bare_cos(next_a)*r, center.y, center.z + bare_sin(next_a)*r };
            p1 = rot(p1); p2 = rot(p2);
            if (p1.z + camera_z_offset <= 0.1f || p2.z + camera_z_offset <= 0.1f) continue;
            Vec2 v1 = Project3D(p1, cx, cy); Vec2 v2 = Project3D(p2, cx, cy);
            if (v1.x != -1 && v2.x != -1) DrawLine(v1.x, v1.y, v2.x, v2.y, color);
        }
    }
}

void DrawDome3D(Vec3 center, float radius, uint32_t color, float alpha, int cx, int cy) {
    float sy = bare_sin(global_cam_rot_y); float cy_rot = bare_cos(global_cam_rot_y);
    float sx = bare_sin(global_cam_rot_x); float cx_rot = bare_cos(global_cam_rot_x);
    auto rot = [&](Vec3 p) -> Vec3 {
        float nx = p.x * cy_rot - p.z * sy; float nz = p.x * sy + p.z * cy_rot; p.x = nx; p.z = nz;
    ...
    auto inv_rot = [&](Vec3 c) -> Vec3 {
        Vec3 p = c;
        float ny = p.y * cx_rot + p.z * sx; float nz = -p.y * sx + p.z * cx_rot; p.y = ny; p.z = nz;
        float nx = p.x * cy_rot + p.z * sy; nz = -p.x * sy + p.z * cy_rot; p.x = nx; p.z = nz; return p;
    };
    
    Vec3 cam_center = rot(center);
    
    for (int ring = 0; ring < 20; ring++) { // 20 pixel dicker Atmosphären-Glow
        float r = radius - (ring * 1.5f); // Etwas breiter fächern
        float current_alpha = alpha * (1.0f - ((float)ring / 20.0f)); // Nach innen ausfaden
        
        for (float a = 0.0f; a < 6.28318f; a += 0.01f) {
            float next_a = a + 0.01f;
            // 1. Basis-Ring (liegt flach auf der Erde)
            Vec3 bp1 = { center.x + (bare_cos(a)*r), center.y, center.z + (bare_sin(a)*r) };
            Vec3 bp2 = { center.x + (bare_cos(next_a)*r), center.y, center.z + (bare_sin(next_a)*r) };
            Vec3 bc1 = rot(bp1); Vec3 bc2 = rot(bp2);
            if (bc1.z + camera_z_offset > 0.1f && bc2.z + camera_z_offset > 0.1f) {
                Vec2 v1 = Project3D(bc1, cx, cy); Vec2 v2 = Project3D(bc2, cx, cy);
                if (v1.x != -1 && v2.x != -1) DrawLineAlpha(v1.x, v1.y, v2.x, v2.y, color, current_alpha);
            }
            
            // 2. Silhouette (Umriss der Kuppel gegen den Himmel, berechnet im Kamera-Raum!)
            Vec3 sp1 = { cam_center.x + (bare_cos(a)*r), cam_center.y + (bare_sin(a)*r), cam_center.z };
            Vec3 sp2 = { cam_center.x + (bare_cos(next_a)*r), cam_center.y + (bare_sin(next_a)*r), cam_center.z };
            Vec3 sw1 = inv_rot(sp1); Vec3 sw2 = inv_rot(sp2);
            
            // Nur zeichnen, wenn oberhalb der Basis! (Welt y >= center.y)
            if (sw1.y >= center.y - 1.0f && sw2.y >= center.y - 1.0f) {
                if (sp1.z + camera_z_offset > 0.1f && sp2.z + camera_z_offset > 0.1f) {
                    Vec2 v1 = Project3D(sp1, cx, cy); Vec2 v2 = Project3D(sp2, cx, cy);
                    if (v1.x != -1 && v2.x != -1) DrawLineAlpha(v1.x, v1.y, v2.x, v2.y, color, current_alpha);
                }
            }
        }
...
extern "C" void kernel_main64(BootInfo* boot_info) {
    ...
    */
    ...
    str_cpy(windows[1].title, "APPS");     windows[1].x=0; windows[1].y=300; windows[1].w=650; windows[1].h=450; windows[1].color=0xDDDDDD;
    str_cpy(windows[2].title, "SAVE AS..."); windows[2].x=0; windows[2].y=150; windows[2].w=300; windows[2].h=200; windows[2].color=0xDDDDDD;
    str_cpy(windows[3].title, "SYSTEM");   windows[3].x=0; windows[3].y=0;  windows[3].w=700; windows[3].h=450; windows[3].color=0xFFD700;
    ...
    _114(1) {
        ...
        UpdateAionBackground();
        if (aion_wants_foreground) {
            aion_wants_foreground = false;
            if (!windows[18].open) {
        ...
        if (aion_wants_background) {
            aion_wants_background = false;
            if (windows[18].open) {
                windows[18].minimized = _128;
            }
        }
        ...
        if (key_ready) {
            ...
            _15(!(sc & 0x80)) { /// Taste wurde GEDRÜCKT (Make Code)
                ...
                _15(sc EQ 0x41) os_vsync_enabled = !os_vsync_enabled; // F7 = VSYNC TOGGLE
                ...
                _15(fw AND fw->open AND !fw->minimized) {
                    ...
                    _15(fw->id EQ 3) {
                        _15(sc EQ 0x50) { // Pfeil Runter
                            if (sys_selected_item < 5) sys_selected_item++; 
                        } 
                        ...
                        _15(sc EQ 0x1C) { // ENTER-TASTE
                            ...
                            else if (sys_selected_item == 5) { extern bool os_auto_update; os_auto_update = !os_auto_update; }
                        }
                    }
					// ==========================================
        ...
        if (cfg_bg_mode == 1) {
            ...
            float t_sun = real_h / 24.0f * 2.0f * 3.14159f;
            
            float dome_r = 3000.0f; // Firmament radius angepasst an den korrekten Scale (bis äußeren Nebelrand)
            
            // Kontinuierliche Zeit
            float total_days_abs = (t->tm_year - 50) * 365.25f + t->tm_yday + real_h / 24.0f;
            float cont_t_sun = total_days_abs * 2.0f * 3.14159f;
            
            // 0: Sonne 1 (Mittelerde, Radius ~500)
            float sun_r = 500.0f + bare_sin(t_sun * 4.0f) * 30.0f;
            float sun_y = 200.0f + bare_sin(t_sun * 4.0f) * 20.0f;
            system[0] = { 0, { bare_cos(t_sun) * sun_r, sun_y, bare_sin(t_sun) * sun_r }, 30, 0 };
            
            // 1: Mond 1 (Mittelerde)
            float moon_t = t_sun - (total_days_abs / 29.53f) * 2.0f * 3.14159f;
            float moon_r = 400.0f + bare_sin(moon_t * 4.0f) * 30.0f;
            float moon_y = 150.0f + bare_sin(moon_t * 4.0f) * 20.0f;
            system[1] = { 1, { bare_cos(moon_t) * moon_r, moon_y, bare_sin(moon_t) * moon_r }, 20, 0xDDDDDD };
            
            // 2-8: Sterne / Planeten als Lichter am Firmament
            for (int i = 2; i < 9; i++) {
                int lat = 16 + (i * 10) % 48; 
                int lon = (i * 70) % 256;
                float r = (Cos(lat) * dome_r) / 84.0f;
                float h = (Sin(lat) * dome_r) / 84.0f;
                system[i] = { 1, { (Cos(lon)*r)/84.0f, h - 10.0f, (Sin(lon)*r)/84.0f }, 10, 0xFFFFFF };
            }
        }

        // ==========================================
        // 4. DIE INTERAKTIVE KAMERA (Rechte Maustaste & Mausrad)
        // ==========================================
        ...
        if (mouse_wheel != 0) {
            camera_z_offset -= mouse_wheel * 100.0f;
            if (camera_z_offset < 100.0f) camera_z_offset = 100.0f;
            if (camera_z_offset > 16000.0f) camera_z_offset = 16000.0f;
            mouse_wheel = 0;
        }
        ...
        if (cfg_bg_mode == 1) {
            ...
            auto lerp_color = [](uint32_t c1, uint32_t c2, float t) -> uint32_t {
                uint32_t r = ((c1 >> 16) & 0xFF) * (1 - t) + ((c2 >> 16) & 0xFF) * t;
                uint32_t g = ((c1 >> 8) & 0xFF) * (1 - t) + ((c2 >> 8) & 0xFF) * t;
                uint32_t b = (c1 & 0xFF) * (1 - t) + (c2 & 0xFF) * t;
                return (r << 16) | (g << 8) | b;
            };
            
            uint32_t dome_color = 0;
            if (real_h >= 4.0f && real_h < 6.0f) dome_color = lerp_color(0x050511, 0xFF5522, (real_h - 4.0f)/2.0f); // Night to Sunrise
            else if (real_h >= 6.0f && real_h < 8.0f) dome_color = lerp_color(0xFF5522, 0x44AAFF, (real_h - 6.0f)/2.0f); // Sunrise to Day
            else if (real_h >= 8.0f && real_h < 18.0f) dome_color = 0x44AAFF; // Day
            else if (real_h >= 18.0f && real_h < 20.0f) dome_color = lerp_color(0x44AAFF, 0xFF5522, (real_h - 18.0f)/2.0f); // Day to Sunset
            else if (real_h >= 20.0f && real_h < 22.0f) dome_color = lerp_color(0xFF5522, 0x050511, (real_h - 20.0f)/2.0f); // Sunset to Night
            else dome_color = 0x050511; // Night

            // Ozean-Scheibe (tiefblau mit Eis-Rand)
            DrawDisc3D({0, 0, 0}, 3000.0f, 0x0C3366, 0xCCDDEE, v_cx, v_cy);
            DrawDome3D({0, 0, 0}, 3000.0f, dome_color, 0.05f, v_cx, v_cy); // 95% transparent -> 0.05 alpha
            
            // Kamera-Rotation vorbereiten
            float cy_cam = bare_cos(global_cam_rot_y); float sy_cam = bare_sin(global_cam_rot_y);
            float cx_cam = bare_cos(global_cam_rot_x); float sx_cam = bare_sin(global_cam_rot_x);
            
            auto map_rot = [&](Vec3 p) -> Vec3 {
                float nx = p.x * cy_cam - p.z * sy_cam; float nz = p.x * sy_cam + p.z * cy_cam; p.x = nx; p.z = nz;
                float ny = p.y * cx_cam - p.z * sx_cam; nz = p.y * sx_cam + p.z * cx_cam; p.y = ny; p.z = nz; return p;
            };
            
            // 1. Landflächen füllen (Naturfarben: variiert je nach Breitengrad)
            for(int i = 0; i < num_land_spans; i++) {
                Vec3 p1 = { land_spans[i][0], 0.0f, land_spans[i][1] };
                Vec3 p2 = { land_spans[i][2], 0.0f, land_spans[i][3] };
                // Breitengrad-abhängige Farbe (z-Position = Entfernung vom Nordpol)
                float dist = land_spans[i][1]; // z-Koordinate
                float abs_dist = (dist < 0) ? -dist : dist;
                float norm = abs_dist / 750.0f; // 0=Nordpol, 1=Rand
                uint32_t land_color;
                if (norm > 0.75f) {
                    // Tundra/Eis (nahe Antarktis) - blass grünlich-grau
                    land_color = 0x889988;
                } else if (norm > 0.55f) {
                    // Gemäßigt - saftiges Grün
                    land_color = 0x2D7A3A;
                } else if (norm > 0.30f) {
                    // Subtropisch - wärmeres Grün
                    land_color = 0x3B8C35;
                } else if (norm > 0.15f) {
                    // Tropisch / Wüste - sandbraun bis olivgrün  
                    land_color = 0x7A6B3A;
                } else {
                    // Arktisch (Nordpol-Nähe) - eisig weiß-grau
                    land_color = 0x99AABB;
                }
                p1 = map_rot(p1); p2 = map_rot(p2);
                if (p1.z + camera_z_offset > 0.1f && p2.z + camera_z_offset > 0.1f) {
                    Vec2 v1 = Project3D(p1, v_cx, v_cy); Vec2 v2 = Project3D(p2, v_cx, v_cy);
                    if (v1.x != -1 && v2.x != -1) DrawLine(v1.x, v1.y, v2.x, v2.y, land_color);
                }
            }
            
            // 2. Küstenlinien (dunklere Kontur für Definition)
            uint32_t coast_color = 0x1A4422;
            for(int i = 0; i < num_coast_lines; i++) {
                Vec3 p1 = { coast_lines[i][0], 0.0f, coast_lines[i][1] };
                Vec3 p2 = { coast_lines[i][2], 0.0f, coast_lines[i][3] };
                p1 = map_rot(p1); p2 = map_rot(p2);
                if (p1.z + camera_z_offset > 0.1f && p2.z + camera_z_offset > 0.1f) {
                    Vec2 v1 = Project3D(p1, v_cx, v_cy); Vec2 v2 = Project3D(p2, v_cx, v_cy);
                    if (v1.x != -1 && v2.x != -1) DrawLine(v1.x, v1.y, v2.x, v2.y, coast_color);
                }
            }
            
            // 3. TIERKREIS-STERNZEICHEN (Zodiac) außerhalb des Firmaments
            // Die 12 Sternzeichen sind auf einer großen Kugel um das Firmament verteilt
            float zodiac_radius = 3300.0f; // Größer als die Kuppel (3000)
            float zodiac_height = 800.0f; // Höhe über der Ebene
            static float shimmer_phase = 0.0f;
            shimmer_phase += 0.02f; // Langsame Schimmer-Animation
            
            for (int z = 0; z < NUM_ZODIAC; z++) {
                // Position des Sternbilds auf dem Himmelskreis (30° pro Zeichen)
                // Drehung in Echtzeit: Einmal pro Sterntag (ca. 23h 56m)
                // Die Sterne verschieben sich jeden Tag um ~1 Grad gegenüber der Sonne.
                float t_sun = real_h / 24.0f * 2.0f * 3.14159f;
                float total_days = t->tm_yday + real_h / 24.0f;
                float zodiac_base_angle = t_sun + (total_days / 365.25f) * 6.28318f;
                float angle = (z / 12.0f) * 6.28318f + zodiac_base_angle;
                float center_x = bare_cos(angle) * zodiac_radius;
                float center_z = bare_sin(angle) * zodiac_radius;
                float center_y = zodiac_height + bare_sin(angle * 3.0f) * 50.0f; // Leichte Wellenbewegung
                
                // Lokale Achsen für das Sternbild (tangential zum Kreis)
                float tx = -bare_sin(angle); // Tangente X
                float tz = bare_cos(angle);  // Tangente Z
                float scale = 3.0f; // Skalierung der Sternbilder
                
                // Sterne zeichnen (mit Schimmer-Effekt wie im Wasser!)
                for (int s = 0; s < zodiac[z].num_points; s++) {
                    float sx_off = zodiac[z].points[s].x * scale;
                    float sy_off = zodiac[z].points[s].y * scale;
                    float base_bright = zodiac[z].points[s].brightness;
                    
                    // Wasser-Schimmer: jeder Stern flackert unabhängig
                    float shimmer = bare_sin(shimmer_phase * 3.0f + z * 1.7f + s * 2.3f);
                    float shimmer2 = bare_sin(shimmer_phase * 2.1f + z * 0.9f + s * 3.7f);
                    float brightness = base_bright * (0.4f + 0.6f * ((shimmer + 1.0f) * 0.5f));
                    float size_pulse = 1.0f + 0.3f * ((shimmer2 + 1.0f) * 0.5f);
                    ...
                    Vec3 rotated = map_rot(star_pos);
                    if (rotated.z + camera_z_offset > 0.1f) {
                        Vec2 sp = Project3D(rotated, v_cx, v_cy);
                        if (sp.x != -1) {
                            uint32_t intensity = (uint32_t)(brightness * 255.0f);
                            // Leicht bläulich-weiß wie Sterne im Wasser
                            uint32_t r_val = (uint32_t)(intensity * 0.85f);
                            uint32_t g_val = (uint32_t)(intensity * 0.9f);
                            uint32_t b_val = intensity;
                            
                            // Stern-Größe (1-3 Pixel je nach Helligkeit und Puls)
                            int star_size = (int)(base_bright * 2.0f * size_pulse);
                            ...
                            for (int dy = -star_size; dy <= star_size; dy++) {
                                for (int dx = -star_size; dx <= star_size; dx++) {
                                    if (dx*dx + dy*dy <= star_size*star_size) {
                                        float dist_frac = 1.0f - (float)(dx*dx + dy*dy) / (float)(star_size*star_size + 1);
                                        uint32_t pr = (uint32_t)(r_val * dist_frac);
                                        uint32_t pg = (uint32_t)(g_val * dist_frac);
                                        uint32_t pb = (uint32_t)(b_val * dist_frac);
                                        PutPixelAlpha((uint32_t)(sp.x + dx), (uint32_t)(sp.y + dy), (pr << 16)|(pg << 8)|pb, brightness * dist_frac);
                                    }
                                }
                            }
                        }
                ...
                // Verbindungslinien zwischen den Sternen (sehr schwach, schimmernd)
                for (int l = 0; l < zodiac[z].num_lines; l++) {
                    int from = zodiac[z].lines[l].from;
                    int to = zodiac[z].lines[l].to;
                    
                    float sx1 = zodiac[z].points[from].x * scale;
                    float sy1 = zodiac[z].points[from].y * scale;
                    float sx2 = zodiac[z].points[to].x * scale;
                    float sy2 = zodiac[z].points[to].y * scale;
                    
                    Vec3 lp1 = { center_x + tx*sx1, center_y + sy1, center_z + tz*sx1 };
                    Vec3 lp2 = { center_x + tx*sx2, center_y + sy2, center_z + tz*sx2 };
                    
                    lp1 = map_rot(lp1); lp2 = map_rot(lp2);
                    if (lp1.z + camera_z_offset > 0.1f && lp2.z + camera_z_offset > 0.1f) {
                        Vec2 v1 = Project3D(lp1, v_cx, v_cy); Vec2 v2 = Project3D(lp2, v_cx, v_cy);
                        if (v1.x != -1 && v2.x != -1) {
                            // Schimmer-Alpha für die Linien
                            float line_shimmer = bare_sin(shimmer_phase * 1.5f + z * 2.0f + l * 1.1f);
                            float line_alpha = 0.08f + 0.07f * ((line_shimmer + 1.0f) * 0.5f);
                            DrawLineAlpha(v1.x, v1.y, v2.x, v2.y, 0x6688CC, line_alpha);
                        }
                    }
                }
            }
        ...
        if (cfg_bg_mode == 1) {
            ...
            for(int i=0; i<9; i++) {
                ...
                if (system[i].color == 0xDDDDDD) moon2d = Project3D(system[i].pos, v_cx, v_cy);
            }
            
            if (moon2d.x != -1) {
                // Mittig auf den Mond zeichnen
                TextC(moon2d.x, moon2d.y - 4, fps_buffer, 0x000000, _128); 
            }
        } else {
            ...
            _15(win->open && !win->minimized) { 
                ...
                _15(galaxy_expansion >= 100) { 
                    ...
                    _15(planets[i].dist > 50) { 
                        // Real-time orbit speeds based on actual orbital periods (in seconds)
                        float speeds[6] = {
                            (2.0f * 3.14159f) / (88.0f * 86400.0f),    // Mercury
                            (2.0f * 3.14159f) / (225.0f * 86400.0f),   // Venus
                            (2.0f * 3.14159f) / (365.25f * 86400.0f),  // Earth
                            (2.0f * 3.14159f) / (687.0f * 86400.0f),   // Mars
                            (2.0f * 3.14159f) / (4333.0f * 86400.0f),  // Jupiter
                            (2.0f * 3.14159f) / (10759.0f * 86400.0f)  // Saturn
                        };
                        time_t current_t = time(NULL);
                        long delta_t = (long)current_t - 1786557600L; // 2026-08-12 18:00:00 UTC
                        planets[i].real_ang = (delta_t * speeds[i]);
                    } 
                } _41 { 
        ...
        if (aion_requested_app != -1) {
            int aid = aion_requested_app;
            if (aid >= 0 && aid < 20) {
                windows[aid].open = _128;
                windows[aid].minimized = _86;
                focus_window(aid);
            }
            aion_requested_app = -1;
        }
        ...
        _39(_43 i=0; i<50; i++) {
            ...
            DrawRoundedRect(wx, wy, ww, 30, 0, 0x333333);
            
            /// Rahmen & Highlights
            DrawRoundedRect(wx+12, wy, ww-24, 1, 0, 0x999999);
            DrawRoundedRect(wx, wy+12, 1, wh-24, 0, 0x999999);
            _89 txt_color = 0xFFFFFF;
            Text(wx+15, wy+15, win->title, txt_color, _128);
            /// Fenster-Buttons
            _43 bx = wx + ww/2; 
            Text(bx-60, wy+15, "MIN", 0xAAAAAA, _128);
            Text(bx-10, wy+15, "FULL", 0xAAAAAA, _128); 
            Text(bx+50, wy+15, "X", 0xFF5555, _128);
            /// =========================================================
			...
			/// =========================================================
            /// APP: EARTHQUAKE (ID 17)
            /// =========================================================
            _15(win->id EQ 17) {
                UpdateEarthquakeApp(wx, wy, ww, wh, !blocked);
            }
            /// =========================================================
            /// APP: AION (ID 18)
            /// =========================================================
            _15(win->id EQ 18) {
                ...
                if (aion_use_3d_avatar) {
                    TextC(wx + ww - 60, wy + 8, "MODE: 3D", 0x00FF00, _128);
                } else {
                    TextC(wx + ww - 60, wy + 8, "MODE: 2D", 0x00FF00, _128);
                }
                
                if (!blocked && mouse_just_pressed && input_cooldown == 0 && is_over_rect(mouse_x, mouse_y, wx + ww - 110, wy + 5, 100, 20)) {
                ...
                if (!blocked && mouse_just_pressed) {
                    aion_input_focus = is_over_rect(mouse_x, mouse_y, wx+10, wy+wh-35, ww-100, 25);
                }
                
                extern char os_text_input_buf[256];
                extern bool os_text_input_enter;
                
                if (aion_input_focus && os_text_input_enter) {
                    if (os_text_input_buf[0] != 0) {
                        AionHandleInput(os_text_input_buf);
                        os_text_input_buf[0] = 0;
                    }
                    os_text_input_enter = false;
                }
                if (!aion_input_focus) {
                    os_text_input_enter = false;
                }
                
                extern int aion_lang;
                extern bool aion_teach_mode;
                
                // LANG Button
                bool lang_hover = is_over_rect(mouse_x, mouse_y, wx+10, wy+wh-65, 80, 25);
                DrawRoundedRect(wx+10, wy+wh-65, 80, 25, 4, lang_hover ? 0x666666 : 0x444444);
                char lang_str[32];
                if (aion_lang == 0) str_cpy(lang_str, "LANG: EN");
                else if (aion_lang == 1) str_cpy(lang_str, "LANG: DE");
                else if (aion_lang == 2) str_cpy(lang_str, "LANG: KO");
                TextC(wx+50, wy+wh-60, lang_str, 0xFFFFFF, _128);
                
                static bool lang_clicked_last = false;
                bool lang_clicked_now = (lang_hover && mouse_down);
                if (lang_clicked_now && !lang_clicked_last) {
                    aion_lang = (aion_lang + 1) % 3;
                }
                lang_clicked_last = lang_clicked_now;
                
                // TEACH Button
                bool teach_hover = is_over_rect(mouse_x, mouse_y, wx+100, wy+wh-65, 120, 25);
                uint32_t teach_color = aion_teach_mode ? 0x338833 : 0x662222;
                if (teach_hover) teach_color = aion_teach_mode ? 0x55AA55 : 0x883333;
                DrawRoundedRect(wx+100, wy+wh-65, 120, 25, 4, teach_color);
                char teach_str[32];
                if (aion_teach_mode) str_cpy(teach_str, "TEACH: ON");
                else str_cpy(teach_str, "TEACH: OFF");
                TextC(wx+160, wy+wh-60, teach_str, 0xFFFFFF, _128);
                
                static bool teach_clicked_last = false;
                bool teach_clicked_now = (teach_hover && mouse_down);
                if (teach_clicked_now && !teach_clicked_last) {
                    aion_teach_mode = !aion_teach_mode;
                }
                teach_clicked_last = teach_clicked_now;

                DrawRoundedRect(wx+10, wy+wh-35, ww-120, 25, 4, aion_input_focus ? 0x444444 : 0x222222);
                Text(wx+15, wy+wh-30, os_text_input_buf[0] == 0 ? "Ask AION something..." : os_text_input_buf, os_text_input_buf[0] == 0 ? 0x555555 : 0xFFFFFF, _128);

                
                // MIC Button
                extern bool aion_auto_listen;
                extern bool aion_is_thinking_exposed;
                bool mic_hover = is_over_rect(mouse_x, mouse_y, wx+ww-150, wy+wh-35, 45, 25);
                uint32_t btn_color = aion_auto_listen ? 0x338833 : 0x662222;
                if (mic_hover) btn_color = aion_auto_listen ? 0x55AA55 : 0x883333;
                DrawRoundedRect(wx+ww-150, wy+wh-35, 45, 25, 4, btn_color);
                TextC(wx+ww-135, wy+wh-30, aion_is_thinking_exposed ? "WAIT" : "AUTO", 0xFFFFFF, _128);
                static bool auto_clicked_last = false;
                bool auto_clicked_now = (mic_hover && mouse_down);
                if (auto_clicked_now && !auto_clicked_last) {
                    extern void AionToggleMic();
                    AionToggleMic();
                }
                auto_clicked_last = auto_clicked_now;
                
                bool hov_s = is_over_rect(mouse_x, mouse_y, wx+ww-100, wy+wh-35, 80, 25);
                DrawRoundedRect(wx+ww-100, wy+wh-35, 80, 25, 4, hov_s ? 0x883333 : 0x662222);
                TextC(wx+ww-60, wy+wh-30, "SEND", 0xFFFFFF, _128);
                
                if (mouse_just_pressed && hov_s) {
                    if (os_text_input_buf[0] != 0) {
                        AionHandleInput(os_text_input_buf);
                        os_text_input_buf[0] = 0;
                    }
            ...
            _15(win->id EQ 19) {
                ...
                // --- SEARCH BAR ---
                _43 search_y = wy + 40;
                DrawRoundedRect(wx + 20, search_y, ww - 40, 30, 4, media_search_active ? 0x333333 : 0x111111);
                char search_text[256];
                if (media_search_active) {
                    snprintf(search_text, 256, "Search: %s_", os_text_input_buf);
                } else {
                    snprintf(search_text, 256, "Search by Name or Genre... (Click here)");
                }
                Text(wx + 30, search_y + 8, search_text, 0xFFFFFF, _128);
                
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx + 20, search_y, ww - 40, 30)) {
                    media_search_active = true;
                    os_text_input_buf[0] = 0;
                    input_cooldown = 15;
                }
                ...
                int num_filtered = 0;
                for(int i=0; i<num_radio_stations; i++) {
                    if (os_text_input_buf[0] == 0 || strstr(radio_stations[i].name, os_text_input_buf) || strstr(radio_stations[i].genre, os_text_input_buf)) {
                ...
                _43 list_y = wy + 80;
                _43 list_h = 200;
                DrawRoundedRect(wx + 20, list_y, ww - 40, list_h, 4, 0x0A0A0A);
                
                int max_visible = 6;
                if (media_scroll_y > num_filtered - max_visible) media_scroll_y = num_filtered - max_visible;
                if (media_scroll_y < 0) media_scroll_y = 0;
                
                for(int i=0; i<max_visible; i++) {
                    int idx = media_scroll_y + i;
                    if (idx >= num_filtered) break;
                    
                    int real_idx = filtered_indices[idx];
                    _43 item_y = list_y + 10 + (i * 30);
                    
                    if (real_idx == current_radio_station) {
                        DrawRoundedRect(wx + 25, item_y, ww - 50, 25, 2, 0x442288);
                    } else if (is_active && is_over_rect(mouse_x, mouse_y, wx + 25, item_y, ww - 50, 25)) {
                        DrawRoundedRect(wx + 25, item_y, ww - 50, 25, 2, 0x222222);
                        if (mouse_just_pressed && input_cooldown == 0) {
                            current_radio_station = real_idx;
                            input_cooldown = 15;
                        }
                    }
                    
                    char row_text[128];
                    snprintf(row_text, 128, "%s [%s] - %s", 
                        radio_stations[real_idx].is_favorite ? "[*]" : "   ", 
                        radio_stations[real_idx].genre, 
                        radio_stations[real_idx].name);
                        
                    Text(wx + 35, item_y + 5, row_text, real_idx == current_radio_station ? 0xFFFFFF : 0xAAAAAA, _128);
                }
                
                // SCROLL BUTTONS
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx + ww - 40, list_y + 10, 20, 20)) {
                    media_scroll_y--; input_cooldown = 5;
                }
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx + ww - 40, list_y + list_h - 30, 20, 20)) {
                    media_scroll_y++; input_cooldown = 5;
                }
                TextC(wx + ww - 30, list_y + 15, "/\\", 0xFFFFFF, _128);
                TextC(wx + ww - 30, list_y + list_h - 25, "\\/", 0xFFFFFF, _128);
                
                // --- CURRENT STATION DISPLAY ---
                _43 station_y = wy + 300;
                char disp_name[128];
                ...
                // PLAY Button
                DrawRoundedRect(wx + 30, ctrl_y, 100, 40, 5, 0x008800);
                TextC(wx + 80, ctrl_y + 12, "PLAY", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx+30, ctrl_y, 100, 40)) {
                    char cmd[512];
                    snprintf(cmd, 512, "killall mpv >/dev/null 2>&1; mpv --input-ipc-server=/tmp/mpvsocket --ytdl=no --no-video '%s' >/tmp/mpv.log 2>&1 &", radio_stations[current_radio_station].url);
                    ::system(cmd);
                ...
                // STOP Button
                DrawRoundedRect(wx + 150, ctrl_y, 100, 40, 5, 0xAA0000);
                TextC(wx + 200, ctrl_y + 12, "STOP", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx+150, ctrl_y, 100, 40)) {
                ...
                // FAVORITE Button
                DrawRoundedRect(wx + 270, ctrl_y, 200, 40, 5, 0xAA8800);
                if (radio_stations[current_radio_station].is_favorite) {
                    TextC(wx + 370, ctrl_y + 12, "- REMOVE FAV", 0xFFFFFF, _128);
                } else {
                    TextC(wx + 370, ctrl_y + 12, "+ ADD FAVORITE", 0xFFFFFF, _128);
                }
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx+270, ctrl_y, 200, 40)) {
                    radio_stations[current_radio_station].is_favorite = !radio_stations[current_radio_station].is_favorite;
                    input_cooldown = 20;
                }
                ...
                // VOL -
                DrawRoundedRect(wx + 30, vol_y, 100, 30, 5, 0x444444);
                TextC(wx + 80, vol_y + 8, "VOL -", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx+30, vol_y, 100, 30)) {
                    ::system("echo '{ \"command\": [\"add\", \"volume\", -5] }' | socat - /tmp/mpvsocket >/dev/null 2>&1");
                    input_cooldown = 10;
                }
                
                // MUTE
                DrawRoundedRect(wx + 150, vol_y, 100, 30, 5, 0x444444);
                TextC(wx + 200, vol_y + 8, "MUTE", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx+150, vol_y, 100, 30)) {
                    ::system("echo '{ \"command\": [\"cycle\", \"mute\"] }' | socat - /tmp/mpvsocket >/dev/null 2>&1");
                    input_cooldown = 20;
                }
                
                // VOL +
                DrawRoundedRect(wx + 270, vol_y, 100, 30, 5, 0x444444);
                TextC(wx + 320, vol_y + 8, "VOL +", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND is_active AND is_over_rect(mouse_x, mouse_y, wx+270, vol_y, 100, 30)) {
                    ::system("echo '{ \"command\": [\"add\", \"volume\", 5] }' | socat - /tmp/mpvsocket >/dev/null 2>&1");
                    input_cooldown = 10;
                }
            }
            ...
            _15(win->id EQ 3) {
                ...
                extern bool os_auto_update;
                uint32_t btn_update_col = os_auto_update ? 0x00AA00 : 0xAA0000;
                if (sys_selected_item == 5) btn_update_col = 0xCCCC00; // highlighted
                
                _43 btn_upd_x = btn_scan_x + 260;
                DrawRoundedRect(btn_upd_x, btn_scan_y, 160, 30, 4, btn_update_col);
                Text(btn_upd_x + 10, btn_scan_y + 8, os_auto_update ? "AUTO-UPDATE: ON" : "AUTO-UPDATE: OFF", 0xFFFFFF, _128);
                
                _44 mouse_klick_upd = (mouse_just_pressed AND is_over_rect(mouse_x, mouse_y, btn_upd_x, btn_scan_y, 160, 30));
                _15(input_cooldown EQ 0 AND is_active AND mouse_klick_upd) {
            ...
            _15(win->id EQ 1) { 
                ...
                // BARE METAL APPS
                // ==========================================
                TextC(wx+ww/2, row1-15, "--- BARE METAL APPS ---", 0x555555, _128);
                
                uint32_t bm_col = 0xAA5500;
                ...
                DrawRoundedRect(col2, row1, bw, bh, 4, bm_col); 
                TextC(col2+bw/2, row1+10, "EARTHQUAKES", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col2, row1, bw, bh)) {
                    if (!windows[17].open) { str_cpy(windows[17].title, "EARTHQUAKES"); windows[17].x = 50; windows[17].y = 30; windows[17].w = 1200; windows[17].h = 800; windows[17].color = 0x050510; windows[17].open = _128; windows[17].minimized = _86; focus_window(17); InitEarthquakeApp(); }
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86;
                }

                DrawRoundedRect(col3, row1, bw, bh, 4, bm_col); 
                TextC(col3+bw/2, row1+10, "COSMOS NETWORK", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col3, row1, bw, bh)) {
                    windows[16].open = _128; windows[16].minimized = _86; str_cpy(windows[16].title, "COSMOS NETWORK"); windows[16].w = 700; windows[16].h = 450; windows[16].x = (screen_w - 700) / 2; windows[16].y = (screen_h - 450) / 2; focus_window(16);
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86;
                }

                // Row 2
                DrawRoundedRect(col1, row2, bw, bh, 4, bm_col); 
                TextC(col1+bw/2, row2+10, "SYSTEM INFO", 0xFFFFFF, _128);
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col1, row2, bw, bh)) {
                    if (!windows[3].open) { str_cpy(windows[3].title, "SYSTEM INFO"); windows[3].x = 50; windows[3].y = 50; windows[3].w = 600; windows[3].h = 400; windows[3].color = 0x222222; windows[3].open = _128; windows[3].minimized = _86; focus_window(3); }
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86;
                }

                DrawRoundedRect(col2, row2, bw, bh, 4, bm_col); 
                TextC(col2+bw/2, row2+10, "BARE METAL DSK", 0xFFFFFF, _128);
                ...
                DrawRoundedRect(col3, row2, bw, bh, 4, bm_col);
                TextC(col3+bw/2, row2+10, "COSMOS DEFENDER", 0xFFFFFF, _128);
                ...
                // Row 3
                DrawRoundedRect(col1, row3, bw, bh, 4, bm_col); 
                TextC(col1+bw/2, row3+10, "PLAY PONG", 0xFFFFFF, _128); 
                ...
                DrawRoundedRect(col2, row3, bw, bh, 4, bm_col);
                TextC(col2+bw/2, row3+10, "PLAY BLOBBY", 0xFFFFFF, _128); 
                ...
                DrawRoundedRect(col3, row3, bw, bh, 4, bm_col);
                TextC(col3+bw/2, row3+10, "EMULATOR", 0xFFFFFF, _128); 
                ...
                // Row 4
                int row4 = row3 + 45;
                DrawRoundedRect(col1, row4, bw, bh, 4, bm_col);
                TextC(col1+bw/2, row4+10, "SMASH CATS 3D", 0xFFFFFF, _128); 
                ...
                // ==========================================
                int row5 = row4 + 60;
                TextC(wx+ww/2, row5-15, "--- LINUX HYBRID APPS ---", 0x555555, _128);
                uint32_t lin_col = 0x0044AA;
                
                DrawRoundedRect(col1, row5, bw, bh, 4, lin_col); 
                TextC(col1+bw/2, row5+10, "WEB EXPLORER", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col1, row5, bw, bh)) { 
#ifdef __linux__
                    pid_t pid = fork(); if (pid == 0) { execlp("firefox", "firefox", NULL); exit(1); }
#endif
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86; 
                }

                DrawRoundedRect(col2, row5, bw, bh, 4, lin_col);
                TextC(col2+bw/2, row5+10, "CMD PROMPT", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col2, row5, bw, bh)) { 
#ifdef __linux__
                ...
                DrawRoundedRect(col3, row5, bw, bh, 4, lin_col);
                TextC(col3+bw/2, row5+10, "RADIO COSMOS", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col3, row5, bw, bh)) { 
#ifdef __linux__
                ...
                int row6 = row5 + 45;
                DrawRoundedRect(col1, row6, bw, bh, 4, lin_col);
                TextC(col1+bw/2, row6+10, "STEAM", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col1, row6, bw, bh)) { 
#ifdef __linux__
                ...
                DrawRoundedRect(col2, row6, bw, bh, 4, lin_col);
                TextC(col2+bw/2, row6+10, "LUTRIS / EA", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col2, row6, bw, bh)) { 
#ifdef __linux__
                ...
                DrawRoundedRect(col3, row6, bw, bh, 4, lin_col); 
                TextC(col3+bw/2, row6+10, "KI ANTIGRAVITY", 0xFFFFFF, _128); 
                _15(input_cooldown EQ 0 AND mouse_just_pressed AND !blocked AND is_over_rect(mouse_x, mouse_y, col3, row6, bw, bh)) { 
#ifdef __linux__
                    if (fork() == 0) { chdir("/opt/meinos/Antigravity IDE"); execl("./antigravity-ide", "antigravity-ide", "--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage", "--user-data-dir=/root/.config/Antigravity IDE", NULL); exit(1); }
#endif
                    input_cooldown = 25; windows[1].open = _86; windows[2].open = _86; 
                }
}
            ...
            _15(win->id EQ 16) {
                ...
                static bool os2_shift_held = false;
                if (os2_key_scancode == 0x2A || os2_key_scancode == 0x36) os2_shift_held = true;
                if (os2_key_scancode == (0x2A | 0x80) || os2_key_scancode == (0x36 | 0x80)) os2_shift_held = false;
                
                auto get_mapped_char = [](uint8_t sc) -> char {
                    char c = 0;
                    if (sc >= 0x02 && sc <= 0x0A) c = os2_shift_held ? "!@#$%^&*("[sc - 0x02] : '1' + (sc - 0x02);
                    if (sc == 0x0B) c = os2_shift_held ? ')' : '0';
                    if (sc == 0x39) c = ' ';
                    if (sc >= 0x10 && sc <= 0x19) { c = "qwertyuiop"[sc - 0x10]; if(os2_shift_held) c -= 32; }
                    if (sc >= 0x1E && sc <= 0x26) { c = "asdfghjkl"[sc - 0x1E]; if(os2_shift_held) c -= 32; }
                    if (sc >= 0x2C && sc <= 0x32) { c = "zxcvbnm"[sc - 0x2C]; if(os2_shift_held) c -= 32; }
                    if (sc == 0x33) c = os2_shift_held ? '<' : ',';
                    if (sc == 0x34) c = os2_shift_held ? '>' : '.';
                    if (sc == 0x35) c = os2_shift_held ? '?' : '/';
                    if (sc == 0x27) c = os2_shift_held ? ':' : ';';
                    if (sc == 0x28) c = os2_shift_held ? '"' : '\'';
                    if (sc == 0x0C) c = os2_shift_held ? '_' : '-';
                    if (sc == 0x0D) c = os2_shift_held ? '+' : '=';
                    return c;
                };
                ...
                if (zt_input_focus && os2_key_scancode != 0 && input_cooldown == 0) {
                    if (os2_key_scancode == 0x0E && zt_target_id_len > 0) { // Backspace
                        ...
                        char c = get_mapped_char(os2_key_scancode);
                ...
                if (frame % 600 == 0) zt_poll_status(); // Poll every 10 seconds to fix lag
                chat_poll();
            }
        ...
        /// ==========================================
        /// GENERIC TASKBAR (FOR ALL MINIMIZED WINDOWS)
        /// ==========================================
        {
            int min_count = 0;
            int min_ids[32];
            ...
            if(min_count > 0) {
                int total_width = min_count * 70;
                int start_x = (screen_w / 2) - (total_width / 2) + 35;
                int start_y = screen_h - 40;
                for(int j=0; j<min_count; j++) {
                    ...
                    uint32_t p_col = 0x555555; // Default generic color
                    if(id == 17) p_col = 0xAA2200;
                    else if(id == 18) p_col = 0x00FF33;
                    else if(id == 3) p_col = 0xFFFF00;
                    
                    if(hov) p_col = 0xFFFFFF;
                    
                    DrawRoundedRect(px - 20, py - 20, 40, 40, 20, p_col);
                    
                    // Two moons
                    float t = galaxy_expansion * 0.1f;
                    DrawRoundedRect(px + (bare_cos(t)*30)/1 - 3, py + (bare_sin(t)*30)/1 - 3, 6, 6, 3, 0xDDDDDD);
                    DrawRoundedRect(px + (bare_cos(t+3.14f)*30)/1 - 3, py + (bare_sin(t+3.14f)*30)/1 - 3, 6, 6, 3, 0xDDDDDD);
                    
                    // Label
                    char label[16];
                    snprintf(label, sizeof(label), "ID %d", id);
                    TextC(px, py-4, label, 0x000000, _128);
                    
                    if(hov && mouse_just_pressed) {
                        windows[id].minimized = _86;
                        focus_window(id);
                        click_consumed = _128;
                    }
                }
		...
		if (aion_prediction[0] != '\0') {
		    int lines = 1;
		    for (int i=0; aion_prediction[i] != '\0'; i++) if (aion_prediction[i] == '\n') lines++;
		    
		    int box_w = 400;
		    int box_h = 20 + lines * 20;
		    int box_x = screen_w - box_w - 20;
		    int box_y = screen_h - box_h - 20;
		    
		    DrawRoundedRect(box_x, box_y, box_w, box_h, 5, 0x112233);
		    DrawChar(box_x + 10, box_y + 12, 'A', 0x00AAFF, _128);
		    DrawChar(box_x + 18, box_y + 12, 'I', 0x00AAFF, _128);
		    
		    char line_buf[100];
		    int buf_idx = 0;
		    int current_y = box_y + 16;
		    for (int i=0; aion_prediction[i] != '\0'; i++) {
		        if (aion_prediction[i] == '\n') {
		            line_buf[buf_idx] = '\0';
		            Text(box_x + 35, current_y, line_buf, 0xEEEEEE, _86);
		            current_y += 20;
		            buf_idx = 0;
		        } else {
		            if (buf_idx < 99) line_buf[buf_idx++] = aion_prediction[i];
		        }
		    }
		    line_buf[buf_idx] = '\0';
		    Text(box_x + 35, current_y, line_buf, 0xEEEEEE, _86);
		...
		extern bool os_auto_update;
		static int os_auto_update_timer = 0;
		if (os_auto_update) {
		    os_auto_update_timer++;
		    if (os_auto_update_timer > 36000) { // roughly every 10 minutes (at 60FPS)
		        os_auto_update_timer = 0;
		        ::system("./update_os.sh > /tmp/auto_update.log 2>&1 &");
		    }
		} else {
		    os_auto_update_timer = 0;
		}
        ...
        if (windows[18].open) {
            aion_window_open = 1;
            aion_window_x = windows[18].fullscreen ? 0 : windows[18].x;
            aion_window_y = windows[18].fullscreen ? 0 : windows[18].y;
            aion_window_w = windows[18].fullscreen ? screen_w : windows[18].w;
            aion_window_h = windows[18].fullscreen ? screen_h : windows[18].h;
            
            extern int aion_avatar_screen_x;
            extern int aion_avatar_screen_y;
            extern float aion_avatar_zoom;
            bool over_aion_window = !windows[18].minimized && mouse_y > aion_window_y + 30 && mouse_y < aion_window_y + aion_window_h - 220 && mouse_x > aion_window_x && mouse_x < aion_window_x + aion_window_w;
            ...
            if (!is_modal_blocked_global && (over_aion_window || over_avatar_frame || aion_mouse_dragging)) {
                ...
                if (mouse_down && !mouse_right_down) {
                    ...
                    extern float aion_avatar_pan_x;
                    extern float aion_avatar_pan_y;
                    aion_avatar_pan_x += (float)mdx * 0.03f;
                    aion_avatar_pan_y -= (float)mdy * 0.03f;
                }
