...
/// --- SHIVA SHIELD ---
_43 shiva_shield_active = 0;      /// 0 = aus, 1 = an
_43 shiva_shield_phase = 0;       /// Aktuelle Shiva-Phase (0=VOID, 1=MORGENSTERN, 2=ERKENNUNG, 3=HITZE, 4=COLLAPS, 5=RUHE, 6=LEVEL0, 7=NEUGEBURT)
_43 shiva_shield_timer = 0;       /// Frame-Counter für Phase-Timing
_43 shiva_shield_blocked = 0;     /// Angriffe geblockt in diesem Zyklus
_43 shiva_shield_total = 0;       /// Gesamtzahl aller geblockten Angriffe
_94 shiva_shield_word = 0;        /// Das 64-Bit Shiva-Wort (Bit-Visualisierung)
_43 shiva_shield_bit = 63;        /// Aktuelles Bit (MSB → LSB)
_43 shiva_shield_cycle_active = 0;/// 1 = Shiva-Zyklus läuft gerade (nach Angriffs-Block)
static _71 _30* shiva_phase_names[] = {"VOID", "MORGENSTERN", "ERKENNUNG", "HITZE", "COLLAPS", "RUHE", "LEVEL 0", "NEUGEBURT"};
...
void hs_add_to_radar(const char* ip_str, const char* proto) {
        ...
        if(hs_radar[i].ip[0] == ip1 && hs_radar[i].ip[1] == ip2 &&
           hs_radar[i].ip[2] == ip3 && hs_radar[i].ip[3] == ip4) {
            ...
            if (hs_radar[i].isFriend == 0 && !str_starts(hs_radar[i].threat, "BLOCKED")) {
                extern bool os_aion_heuristic;
                bool heuristic_triggered = (os_aion_heuristic && hs_radar[i].hits > 5);
                
                if (hs_radar[i].hits > 20 || heuristic_triggered) {
                    str_cpy(hs_radar[i].threat, "CRITICAL");
                    if (hs_auto_suppression || heuristic_triggered) {
                        char cmd[128];
                        /// Standard DROP-Regel
                        sprintf(cmd, "iptables -A INPUT -s %u.%u.%u.%u -j DROP 2>/dev/null", ip1, ip2, ip3, ip4);
#ifdef __linux__
                        system(cmd);
#endif
                        /// ★ SHIVA SHIELD: Erweiterte Verteidigung ★
                        if (shiva_shield_active) {
                            /// Rate-Limiting: Max 1 Paket/Min von dieser IP (loggen)
                            sprintf(cmd, "iptables -I INPUT -s %u.%u.%u.%u -m limit --limit 1/min -j LOG --log-prefix \"SHIVA-BLOCK: \" 2>/dev/null", ip1, ip2, ip3, ip4);
#ifdef __linux__
                            system(cmd);
#endif
                            /// Connection-Limiting: Max 3 gleichzeitige Verbindungen
                            sprintf(cmd, "iptables -I INPUT -s %u.%u.%u.%u -m connlimit --connlimit-above 3 -j REJECT 2>/dev/null", ip1, ip2, ip3, ip4);
#ifdef __linux__
                            system(cmd);
#endif
                            /// Shiva-Zyklus starten
                            shiva_shield_cycle_active = 1;
                            shiva_shield_phase = 0;
                            shiva_shield_timer = 0;
                            shiva_shield_bit = 63;
                            shiva_shield_word = 0;
                            shiva_shield_blocked++;
                            shiva_shield_total++;
                            char lb[128];
                            if (heuristic_triggered) sprintf(lb, "AION HEURISTIC: TARGET %u.%u.%u.%u ZERO-DAY BLOCKED!", ip1, ip2, ip3, ip4);
                            else sprintf(lb, "SHIVA: TARGET %u.%u.%u.%u NEUTRALIZED!", ip1, ip2, ip3, ip4);
                            hs_add_log(lb);
                        } _41 {
                            char lb[128];
                            if (heuristic_triggered) sprintf(lb, "AION HEURISTIC: AUTO-SUPPRESSED ZERO-DAY %u.%u.%u.%u!", ip1, ip2, ip3, ip4);
                            else sprintf(lb, "SYSTEM: AUTO-SUPPRESSED TARGET %u.%u.%u.%u!", ip1, ip2, ip3, ip4);
                            hs_add_log(lb);
                        }
    ...
    if (hs_radar_count < 100) {
        ...
        char log_buf[128];
        if (isF) {
            str_cpy(log_buf, "FRIEND DETECTED: ");
        } else {
...
_50 run_holyspirit(_43 wx, _43 wy, _43 ww, _43 wh, _44 blocked) {
    ...
    /// ★ SHIVA SHIELD BUTTON ★
    _43 shiva_btn_x = wx + 10;
    _43 shiva_btn_y = py + 10;
    _43 shiva_btn_w = 130;
    _43 shiva_btn_h = 20;
    _44 shiva_hov = is_over_rect(mouse_x, mouse_y, shiva_btn_x, shiva_btn_y, shiva_btn_w, shiva_btn_h);
    
    _15(shiva_shield_active) {
        /// Pulsierendes Gold wenn aktiv
        _43 pulse = 0xCC8800 + ((frame % 30) < 15 ? 0x331100 : 0x000000);
        DrawRoundedRect(shiva_btn_x, shiva_btn_y, shiva_btn_w, shiva_btn_h, 2, pulse);
        Text(shiva_btn_x + 5, shiva_btn_y + 5, "SHIVA ACTIVE", 0xFFFFFF, _128);
    } _41 {
        DrawRoundedRect(shiva_btn_x, shiva_btn_y, shiva_btn_w, shiva_btn_h, 2, shiva_hov ? 0x550022 : 0x330011);
        Text(shiva_btn_x + 10, shiva_btn_y + 5, "SHIVA: OFF", 0x888888, _128);
    }
    _15(shiva_hov AND mouse_just_pressed AND !blocked AND input_cooldown EQ 0) {
        shiva_shield_active = !shiva_shield_active;
        _15(shiva_shield_active) {
            hs_add_log("SHIVA SHIELD: ACTIVATED!");
            shiva_shield_blocked = 0;
        } _41 {
            hs_add_log("SHIVA SHIELD: DEACTIVATED.");
            shiva_shield_cycle_active = 0;
        }
        input_cooldown = 15;
    }
    ...
    _43 vpn_btn_w = 130;
    _43 vpn_btn_h = 20;
    _44 vpn_hov = is_over_rect(mouse_x, mouse_y, vpn_btn_x, vpn_btn_y, vpn_btn_w, vpn_btn_h);
    
    _15(is_cosmos_vpn_active()) {
        DrawRoundedRect(vpn_btn_x, vpn_btn_y, vpn_btn_w, vpn_btn_h, 2, 0x00AA00);
        Text(vpn_btn_x + 5, vpn_btn_y + 5, "26-B VPN ACTIVE", 0xFFFFFF, _128);
    } _41 {
        DrawRoundedRect(vpn_btn_x, vpn_btn_y, vpn_btn_w, vpn_btn_h, 2, vpn_hov ? 0x0055AA : 0x002255);
        Text(vpn_btn_x + 5, vpn_btn_y + 5, "ACTIVATE VPN", 0xAAAAAA, _128);
    }
    _15(vpn_hov AND mouse_just_pressed AND !blocked AND input_cooldown EQ 0) {
        if (is_cosmos_vpn_active()) {
            deactivate_cosmos_vpn();
        } else {
            activate_cosmos_vpn("142.93.171.120"); // DigitalOcean VPN Server
        }
        input_cooldown = 15;
    }
    
    /// ★ SHIVA PHASE TICKER ★
    _15(shiva_shield_cycle_active) {
        shiva_shield_timer++;
        /// Phase-Dauern in Frames (~30fps): VOID=90, MORGEN=150, ERKENN=36, HITZE=120, COLLAPS=60, RUHE=90, LVL0=75, NEU=24
        _43 phase_frames[] = {90, 150, 36, 120, 60, 90, 75, 24};
        _15(shiva_shield_timer >= phase_frames[shiva_shield_phase]) {
            shiva_shield_timer = 0;
            shiva_shield_phase++;
            /// Bit-Kaskade: Setze Bits von MSB nach LSB während MORGENSTERN
            _15(shiva_shield_phase == 1) { shiva_shield_bit = 63; shiva_shield_word = 0; }
            /// Level 0: Bits löschen
            _15(shiva_shield_phase == 6) { shiva_shield_word = 0; shiva_shield_bit = 63; }
            /// Zyklus Ende
            _15(shiva_shield_phase >= 8) {
                shiva_shield_phase = 0;
                shiva_shield_cycle_active = 0;
            }
        }
        /// Bits setzen während MORGENSTERN Phase
        _15(shiva_shield_phase == 1 AND shiva_shield_bit >= 0) {
            _43 bits_per_frame = 2;  /// ~64 Bits in 32 Frames
            _39(_43 b = 0; b < bits_per_frame AND shiva_shield_bit >= 0; b++) {
                shiva_shield_word |= (1ULL << shiva_shield_bit);
                shiva_shield_bit--;
            }
        }
    }
    ...
    _15(hs_state EQ 0) {
        ...
        /// Shiva Shield: Goldener Schutzring um den Radar
        _15(shiva_shield_active AND shiva_shield_cycle_active) {
            _43 ring_pulse = 60 + ((frame % 40) < 20 ? (frame % 20) : (20 - frame % 20));
            DrawRoundWindow(cx, cy, ring_pulse + 5, 0x332200);
            DrawRoundWindow(cx, cy, ring_pulse, 0xCC8800);
            /// Flammen-Effekt bei aktivem Zyklus
            _15(shiva_shield_phase == 3) {
                _39(_43 fl = 0; fl < 8; fl++) {
                    _43 fx = cx + ((fl * 37 + frame * 3) % 120) - 60;
                    _43 fy = cy - 30 - ((frame + fl * 7) % 25);
                    DrawRoundWindow(fx, fy, 3, 0xFF4400);
                }
            }
        }
        ...
        /// Radar Label + Shiva Status
        _15(shiva_shield_active AND shiva_shield_cycle_active) {
            _30 phase_buf[48];
            str_cpy(phase_buf, "SHIVA: ");
            str_cat(phase_buf, (char*)shiva_phase_names[shiva_shield_phase]);
            TextC(cx, cy+70, phase_buf, 0xFFAA00, _128);
        } _41 {
        ...
        _39(int i=0; i<hs_radar_count && i<5; i++) {
            ...
            bool is_vpn = (hs_radar[i].ip[0] == 142 && hs_radar[i].ip[1] == 93 && hs_radar[i].ip[2] == 171 && hs_radar[i].ip[3] == 120) ||
                          (hs_radar[i].ip[0] == 0) || str_starts(hs_radar[i].proto, "UDP");
            
            if (is_vpn) {
                str_cat(out_buf, "COSMOS VPN [26-BYTE]");
            } else {
                _30 ip_str[16];
            ...
            str_cat(out_buf, hs_radar[i].isFriend ? "] PKTS:" : "] HITS:");
            ...
            if (is_vpn) txt_col = 0xFFD700; // Bright Gold
        ...
        _15(hs_selected_target >= 0 AND hs_selected_target < hs_radar_count) {
            ...
            _15(btn_block_hov AND mouse_just_pressed AND !blocked AND input_cooldown EQ 0) {
                _30 cmd[128];
                _15(is_blocked) {
                    str_cpy(cmd, "iptables -D INPUT -s "); str_cat(cmd, ip_str); str_cat(cmd, " -j DROP 2>/dev/null");
#ifdef __linux__
                    system(cmd);
                    _30 shiva_cmd[256];
                    sprintf(shiva_cmd, "iptables -D INPUT -s %s -m limit --limit 1/min -j LOG --log-prefix \"SHIVA-BLOCK: \" 2>/dev/null", ip_str);
                    system(shiva_cmd);
                    sprintf(shiva_cmd, "iptables -D INPUT -s %s -m connlimit --connlimit-above 3 -j REJECT 2>/dev/null", ip_str);
                    system(shiva_cmd);
#endif
                    str_cpy(hs_radar[hs_selected_target].threat, "SAFE");
                    char lb[128];
                    str_cpy(lb, "SYSTEM: TARGET "); str_cat(lb, ip_str); str_cat(lb, " UNBLOCKED!");
                    hs_add_log(lb);
                } _41 {
                    str_cpy(cmd, "iptables -A INPUT -s "); str_cat(cmd, ip_str); str_cat(cmd, " -j DROP 2>/dev/null");
#ifdef __linux__
                    system(cmd);
#endif
                    str_cpy(hs_radar[hs_selected_target].threat, "BLOCKED");
                    char lb[128];
                    str_cpy(lb, "SYSTEM: TARGET "); str_cat(lb, ip_str); str_cat(lb, " BLOCKED!");
                    hs_add_log(lb);
                }
                input_cooldown = 15;
            }
            ...
            Text(px+15, py_act+65, is_safe ? "MAKE UNSAFE" : "MAKE SAFE", 0xFFFFFF, _86);
            
            _15(btn_safe_hov AND mouse_just_pressed AND !blocked AND input_cooldown EQ 0) {
                _15(is_safe) {
                    hs_radar[hs_selected_target].isFriend = 0;
                    char lb[128];
                    str_cpy(lb, "SYSTEM: TARGET "); str_cat(lb, ip_str); str_cat(lb, " UNSAFE!");
                    hs_add_log(lb);
                } _41 {
...
#ifdef __linux__
                    _30 shiva_cmd[256];
                    sprintf(shiva_cmd, "iptables -D INPUT -s %s -j DROP 2>/dev/null", ip_str);
                    system(shiva_cmd);
                    sprintf(shiva_cmd, "iptables -D INPUT -s %s -m limit --limit 1/min -j LOG --log-prefix \"SHIVA-BLOCK: \" 2>/dev/null", ip_str);
                    system(shiva_cmd);
                    sprintf(shiva_cmd, "iptables -D INPUT -s %s -m connlimit --connlimit-above 3 -j REJECT 2>/dev/null", ip_str);
        ...
        // Log Viewer
        DrawRoundedRect(wx+10, py+180, ww-20, wh-250, 2, 0x111111);
        ...
        // Aion Chat Input
        _43 chat_y = py + wh - 65;
        DrawRoundedRect(wx+10, chat_y, ww-90, 20, 2, 0x222222);
        extern char os_text_input_buf[256];
        extern bool os_text_input_enter;
        Text(wx+15, chat_y+3, os_text_input_buf[0] == 0 ? "Ask AION..." : os_text_input_buf, 0xFFFFFF, _86);
        _15(!blocked AND (frame / 20) % 2 EQ 0) {
            _43 txt_len = 0; while(os_text_input_buf[txt_len]) txt_len++;
            DrawChar(wx+15+(txt_len*7), chat_y+3, '_', 0xFFFFFF, _128);
        }
        
        _44 chat_btn = is_over_rect(mouse_x, mouse_y, wx+ww-70, chat_y, 60, 20);
        DrawRoundedRect(wx+ww-70, chat_y, 60, 20, 2, chat_btn ? 0x00AA00 : 0x005500);
        Text(wx+ww-58, chat_y+3, "SEND", 0xFFFFFF, _128);
        
        extern _50 AionHandleInput(const char* text);
        _15(!blocked AND input_cooldown EQ 0 AND ((chat_btn AND mouse_just_pressed) OR os_text_input_enter)) {
            _15(os_text_input_buf[0] != 0) {
                AionHandleInput(os_text_input_buf);
                hs_add_log("AION: Thinking...");
        ...
        /// ★ SHIVA 64-BIT WORD VISUALISIERUNG ★
        _15(shiva_shield_active) {
            _43 bit_y = py + wh - 55;
            _43 bit_size = 4;
            _43 total_w = 64 * (bit_size + 1);
            _43 bit_start_x = wx + (ww - total_w) / 2;
            
            _39(_43 bit = 63; bit >= 0; bit--) {
                _43 bx = bit_start_x + (63 - bit) * (bit_size + 1);
                _89 color;
                _15(shiva_shield_word & (1ULL << bit)) {
                    /// Aktives Bit: Gold/Cyan je nach Phase
                    _15(shiva_shield_cycle_active AND shiva_shield_phase == 3) {
                        color = 0xFF4400;  /// Hitze: Rot-Orange
                    } _41 _15(shiva_shield_cycle_active AND shiva_shield_phase >= 4) {
                        color = 0x4444AA;  /// Collaps+: Blau
                    } _41 {
                        color = 0xFFAA00;  /// Standard: Gold
                    }
                } _41 {
                    color = 0x1A1A1A;  /// Inaktives Bit: Dunkel
                }
                DrawRoundedRect(bx, bit_y, bit_size, bit_size, 0, color);
            }
            
            /// Statistik-Zeile
            _30 stat_buf[64];
            str_cpy(stat_buf, "SHIELD: ");
            _30 num[8];
            int_to_str(shiva_shield_total, num);
            str_cat(stat_buf, num);
            str_cat(stat_buf, " BLOCKED | WORD: 0x");
            /// Hex des oberen 32-Bit
            _89 hi = (_89)(shiva_shield_word >> 32);
            _89 lo = (_89)(shiva_shield_word & 0xFFFFFFFF);
            _30 hex[20];
            sprintf(hex, "%08X%08X", hi, lo);
            str_cat(stat_buf, hex);
            Text(bit_start_x, bit_y + bit_size + 3, stat_buf, 0x666666, _86);
        }
    }
}
