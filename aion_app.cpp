...
void UpdateAionBackground() {
    ...
    if (has_pending_aion_response) {
        ...
        if (!current_line.empty()) aion_history.push_back(current_line);
        ...
        std::string piper_model = "/root/coding/AI_Desktop/de_DE-thorsten-high.onnx";
        if (aion_teach_mode) {
            piper_model = "/root/coding/AI_Desktop/de_DE-thorsten-high.onnx";
        } else {
            if (aion_lang == 0) piper_model = "/root/coding/AI_Desktop/voice.onnx";
            else if (aion_lang == 1) piper_model = "/root/coding/AI_Desktop/de_DE-thorsten-high.onnx";
            else if (aion_lang == 2) piper_model = "/root/coding/AI_Desktop/ko_KR-kss-medium.onnx";
        }
...
void AionHandleInput(const char* text) {
    ...
    if (aion_teach_mode) {
        system_prompt += "5. TEACHING MODE: You are a language teacher. Detect what language the user is practicing. Explain and correct them in German (DE). Whenever you use foreign words, ALWAYS write them in German phonetic spelling (e.g. 'An-njong-ha-se-jo') so the German TTS can pronounce them perfectly. NEVER use foreign alphabets like Hangul or Kanji.";
    } else {
        if (aion_lang == 0) system_prompt += "5. Speak in English.";
        else if (aion_lang == 1) system_prompt += "5. Speak in German (DE).";
        else if (aion_lang == 2) system_prompt += "5. Speak ONLY in Korean (Hangul). Do not use English or German, the TTS model only supports Korean.";
    }
    ...
    std::string cmd = "(curl -s --max-time 120 -X POST http://127.0.0.1:11434/api/generate -d @/tmp/aion_request.json > /tmp/aion_tmp.json && mv /tmp/aion_tmp.json /tmp/aion.json) >/dev/null 2>&1 &";
    system(cmd.c_str());
}
