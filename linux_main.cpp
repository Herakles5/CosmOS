...
// GPU Fog Ring: Kamera-Parameter aus kernel_main_linux.cpp
extern float global_cam_rot_x;
...
// WAV Sound Buffers
Uint32 cat_wav_len[5] = {0};
Uint8 *cat_wav_buf[5] = {NULL};
...
static FILE* aion_record_file = nullptr;
static bool aion_is_recording = false;

extern "C" void linux_start_mic_record() {
    std::lock_guard<std::mutex> lock(linux_capture_mutex);
    if (aion_record_file) { fclose(aion_record_file); aion_record_file = nullptr; }
    aion_record_file = fopen("/tmp/input.raw", "wb");
    aion_is_recording = true;
}

extern "C" void linux_stop_mic_record() {
    std::lock_guard<std::mutex> lock(linux_capture_mutex);
    aion_is_recording = false;
    if (aion_record_file) {
        fclose(aion_record_file);
        aion_record_file = nullptr;
    }
}
...
void audio_capture_callback(void* userdata, Uint8* stream, int len) {
    ...
    if (aion_is_recording && aion_record_file) {
        fwrite(stream, 1, len, aion_record_file);
    }
}
...
void audio_callback(void* userdata, Uint8* stream, int len) {
    ...
    if (meow_timer > 0) {
        ...
        if (meow_type == 0) active_cat_wav_idx = 0; // sweet-kitty-meow.wav
        else if (meow_type == 1) active_cat_wav_idx = 1; // domestic-cat-hungry-meow.wav
        else if (meow_type == 2) active_cat_wav_idx = 2; // little-cat-pain-meow.wav
        else if (meow_type == 3) active_cat_wav_idx = 3; // angry-wild-cat-roar.wav
        else active_cat_wav_idx = 4; // cartoon-little-cat-meow.wav
...
int global_screen_w = 0;
int global_screen_h = 0;
...
int main(int argc, char** argv) {
    ...
    std::thread audio_init_thread([]() {
        ...
        // Load Cat Sounds
        load_and_convert_wav("sounds/sweet-kitty-meow.wav", 0, &have);
        load_and_convert_wav("sounds/domestic-cat-hungry-meow.wav", 1, &have);
        load_and_convert_wav("sounds/little-cat-pain-meow.wav", 2, &have);
        load_and_convert_wav("sounds/angry-wild-cat-roar.wav", 3, &have);
        load_and_convert_wav("sounds/cartoon-little-cat-meow.wav", 4, &have);
    ...
    while (running) {
        ...
        if (os_vsync_enabled != last_vsync) {
            last_vsync = os_vsync_enabled;
            SDL_GL_SetSwapInterval(os_vsync_enabled ? 1 : 0);
        }
        ...
        static bool linux_mdown_last = false;
        
        linux_mdown = (mb & SDL_BUTTON(1)) != 0;
        if (linux_mdown) {
            if (!linux_mdown_last) mouse_just_pressed = true;
        }
        linux_mdown_last = linux_mdown;
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_TEXTINPUT) {
                ...
                int cur_len = 0; while(os_text_input_buf[cur_len]) cur_len++;
                int ev_len = 0; while(event.text.text[ev_len]) ev_len++;
                if (cur_len + ev_len < 255) {
                    for(int i=0; i<ev_len; i++) os_text_input_buf[cur_len+i] = event.text.text[i];
                    os_text_input_buf[cur_len+ev_len] = 0;
                }
            }
            if (event.type == SDL_KEYDOWN) {
                if (event.key.keysym.sym == SDLK_BACKSPACE) {
                    extern char os_text_input_buf[256];
                    int cur_len = 0; while(os_text_input_buf[cur_len]) cur_len++;
                    if (cur_len > 0) os_text_input_buf[cur_len - 1] = 0;
                }
                if (event.key.keysym.sym == SDLK_RETURN) {
                    extern bool os_text_input_enter;
                    os_text_input_enter = true;
                }
        ...
        global_screen_w = boot_info.screen_width;
        global_screen_h = boot_info.screen_height;
        ...
        if (fb != nullptr) {
            ...
            if (cfg_firmament_mode) {
                ...
                // Perspektiv-Projektion passend zur CPU-Kamera aufsetzen
                glMatrixMode(GL_PROJECTION);
                glLoadIdentity();
                float near_p = 1.0f;
                float far_p = 30000.0f; // Erhöht auf 30000, damit der riesige Nebel beim Rauszoomen nicht abgeschnitten wird
                float right_p = near_p * (float)w / (2.0f * camera_fov);
                float top_p = near_p * (float)h / (2.0f * camera_fov);
                glFrustum(-right_p, right_p, -top_p, top_p, near_p, far_p);
                
                // ModelView: Kamera-Offset + Rotation (CPU-Konvention → OpenGL-Konvention)
                glMatrixMode(GL_MODELVIEW);
                glLoadIdentity();
                
                // 4. Kamera Offset (nach hinten = -Z in OpenGL)
                glTranslatef(0.0f, 0.0f, -camera_z_offset);
                // 3. Z-Achse flippen, da CPU +Z nach "vorne" geht, OpenGL aber -Z
                glScalef(1.0f, 1.0f, -1.0f);
                // 2. X-Rotation (CPU-Formel entspricht genau OpenGL)
                glRotatef(global_cam_rot_x * 57.2957795f, 1.0f, 0.0f, 0.0f);
                // 1. Y-Rotation (CPU-Formel entspricht negativer OpenGL-Rotation)
                glRotatef(-global_cam_rot_y * 57.2957795f, 0.0f, 1.0f, 0.0f);
                
                // Culling aus, um sicherzugehen, dass nichts verworfen wird
                glDisable(GL_CULL_FACE);
                
                // Nebel-Ring zeichnen: r = 780 bis 3000 (Ring 2 und 3)
                int segments = 180;
                float r_inner_start = 780.0f;
                float r_outer_end = 3000.0f;
                float r_step = 25.0f;
                
                for (float r = r_inner_start; r < r_outer_end; r += r_step) {
                    float r_next = r + r_step;
                    if (r_next > r_outer_end) r_next = r_outer_end;
                    
                    float fog_t = (r - r_inner_start) / (r_outer_end - r_inner_start);
                    float fog_t_next = (r_next - r_inner_start) / (r_outer_end - r_inner_start);
                    
                    glBegin(GL_TRIANGLE_STRIP);
                    for (int i = 0; i <= segments; i++) {
                        float a = (float)i * 6.28318f / (float)segments;
                        
                        // Nebel-Noise
                        float noise = sinf(a * 7.0f + r * 0.01f) * 0.15f + cosf(a * 13.0f - r * 0.005f) * 0.1f;
                        float noise_next = sinf(a * 7.0f + r_next * 0.01f) * 0.15f + cosf(a * 13.0f - r_next * 0.005f) * 0.1f;
                        
                        // Alpha: innen dünn, außen dicht (erhöht für Sichtbarkeit)
                        float alpha1 = 0.10f + fog_t * fog_t * 0.75f + noise * fog_t;
                        float alpha2 = 0.10f + fog_t_next * fog_t_next * 0.75f + noise_next * fog_t_next;
                        if (alpha1 < 0.05f) alpha1 = 0.05f; if (alpha1 > 0.90f) alpha1 = 0.90f;
                        if (alpha2 < 0.05f) alpha2 = 0.05f; if (alpha2 > 0.90f) alpha2 = 0.90f;
                        
                        // Farbe: Weiß-Grau mit leichtem Blau-Stich + Wirbel
                        float fr = 0.78f + sinf(a * 5.0f) * 0.12f;
                        float fg = 0.82f + cosf(a * 3.0f + r * 0.003f) * 0.10f;
                        float fb = 0.90f + sinf(a * 9.0f) * 0.08f;
                        
                        float ca = cosf(a);
                        float sa = sinf(a);
                        
                        // Vertices mit normalen Z-Koordinaten (wird durch glScalef geflippt)
                        glColor4f(fr, fg, fb, alpha1);
                        glVertex3f(ca * r, 0.0f, sa * r);
                        
                        glColor4f(fr, fg, fb, alpha2);
                        glVertex3f(ca * r_next, 0.0f, sa * r_next);
                    }
                    glEnd();
                }
                ...
                // Projektion zurücksetzen für den Rest
                glMatrixMode(GL_PROJECTION);
                glLoadIdentity();
                glMatrixMode(GL_MODELVIEW);
                glLoadIdentity();
