...
bool sglProject(float objx, float objy, float objz, float* winx, float* winy, float* winz) {
    sglVec3 v = {objx, objy, objz};
    sglVec3 v0 = mat_mul_vec(current_matrix, v);
    float z0 = v0.z + 5.0f;
    if (z0 < 0.1f) return false;
    float fov = 200.0f;
    *winx = vp_x + vp_w/2.0f + (v0.x/z0)*fov;
    *winy = vp_y + vp_h/2.0f - (v0.y/z0)*fov;
    *winz = v0.z;
    return true;
}
